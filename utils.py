...
class trainUtils: # class where noise is added outside of the function
    ...
    def regularizationLsrt(self, outputscale):
        if type(self.lambdal1)==float:
            return self.getLengthscales(outputscale).pow(0.5).abs().sum()*self.lambdal1 # sqrt of (1/l^2)=1/l
        else:
            return 0


    def train(self, model, scaled, outputscale, learning_rate, l1_lr, isRegSqrt, max_epochs, patience, all_lengthscale): #modifK # add output lengthscale
        ...
        for epoch in range(self.max_epochs):
            train_loss = self.train_step(isRegSqrt,outputscale)
            val_loss=self.val_step(isRegSqrt,outputscale)
    ...
    def train_step(self, isRegSqrt,outputscale):
        self.model.train() 
        self.likelihood.train()
        self.optimizer.zero_grad() #
        if isRegSqrt==True:
            train_loss = - self.mll(self.model(self.Xtrain), self.y_train) + self.regularizationLsrt(outputscale)
        else:
    ...
    def val_step(self, isRegSqrt, outputscale):
        self.model.eval() 
        ...
        with torch.no_grad(), gpytorch.settings.eval_cg_tolerance(1.e-1), gpytorch.settings.max_preconditioner_size(100), gpytorch.settings.cholesky_jitter(1e-1), gpytorch.settings.cholesky_max_tries(10):
            if isRegSqrt==True:
                val_loss= - self.mll(self.likelihood(self.model(self.Xtest)), self.y_test) + self.regularizationLsrt(outputscale)
